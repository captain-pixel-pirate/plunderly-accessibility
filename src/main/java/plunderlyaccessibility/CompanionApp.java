...
import plunderlyaccessibility.debug.DebugTreeWriter;
 ...
 *
 * The Puzzle Pirates JVM creates this class through {@code Probe}. It owns the
 * shared output directory, discovers the game window, watches for file-based
 * scan triggers, and hosts feature views such as the duty-report dashboard.
 */
public class CompanionApp {
    /** Shipped version, shown in the window title. Bump on each release. */
    ...
    private final DebugTreeWriter debugTreeWriter = new DebugTreeWriter();
    ...
    public CompanionApp() {
        ...
        showCompanion(dutyReportFeature.initialText(outDir, featureDebugDir(dutyReportFeature)));
    ...
    private static Path resolveDir() {
        ...
        // System property too, so a jar-based run (e.g. local dev loading from
        // dist/plunderly.jar) can redirect output via extra.txt, which can only
        // pass JVM options -- not environment variables.
     ...
     * macOS, {@code %APPDATA%\Plunderly} on Windows, and the XDG data dir on
     * Linux. Windows uses roaming {@code %APPDATA%} (not Local) because the saved
     * history is durable user data meant to follow a roaming profile.
     */
    ...
    private void saveDebugTreeNow(Window window, CompanionFeature feature) {
        try {
            ...
            debugTreeWriter.write(window, target, feature);
    ...
    private void ensureUiBuilt() {
        if (companion == null) {
                ...
                companion.setMinimumSize(new Dimension(760, 520));
    ...
    private void selectFeature(CompanionFeature feature) {
        ...
        if (view != null) {
            ...
            companionText.setText(feature.initialText(outDir, featureDebugDir(feature)));
            companionText.setCaretPosition(0);
        }
        showCardFor(feature);
    }
