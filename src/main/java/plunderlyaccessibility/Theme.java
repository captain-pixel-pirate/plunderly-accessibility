...
/**
 * This class is intentionally self-contained — palette
 * constants plus a few small Swing styling helpers — with no game dependencies,
 * so any UI surface ({@code CompanionApp}, the duty dashboard, future features)
...
public final class Theme {
    ...
    // Core palette — hex values copied from the web app's dark theme.
    ...
    // Resolve the web app's preferred font stacks against locally installed
    // fonts once, falling back to logical families so text always renders.
    ...
    /**
     * Monospaced font for column-aligned report/summary text. The companion's
     * copy-ready output relies on a fixed pitch, so this stays monospaced rather
     * than switching to the proportional UI font.
     */
    ...
    /**
     * A flat, rounded button that draws its own fill so it looks identical
     * across host look-and-feel themes (the host Aqua/Metal L&F otherwise
     * ignores {@code setBackground} on buttons). Used for every push button in
     * the companion so the palette stays consistent.
     */
    ...
    /**
     * A toggle that renders as one slice of a connected "segmented control" —
     * the iOS-style pill where several mutually-exclusive choices share a single
     * outlined capsule and the selected slice is filled. Used for fixed,
     * choose-one groups (My/Crew, date range) so they read differently from the
     * loose filter chips ({@link AccentToggle}) and from action buttons.
     *
     * <p>Lay segments out edge-to-edge (zero gap) via {@link #segmented} and add
     * them to a {@link javax.swing.ButtonGroup} so only one stays selected.
     */
